//
// Created by wojciechsalapatek on 24.04.18.
//
...
namespace moviesubs {
    void ValidateLineMicroDvd(const string &line, regex pattern, int lineNumber, int byFps, int previousStart){
        smatch matches;
        if (!std::regex_match(line, matches, pattern)) throw InvalidSubtitleLineFormat(lineNumber,line);
        if (std::atoi(matches[1].str().c_str()) < previousStart) throw OutOfOrderFrames(lineNumber, line);
        if (std::atoi(matches[1].str().c_str()) > std::atoi(matches[2].str().c_str())) throw SubtitleEndBeforeStart(lineNumber, line);
        if (std::atoi(matches[1].str().c_str()) + byFps < 0) throw NegativeFrameAfterShift(lineNumber,line);
    }
    ...
    void MicroDvdSubtitles::ShiftAllSubtitlesBy(int offset_in_micro_seconds, int frame_per_second, std::istream *in,
        ...
        while (!in->eof()) {
            ...
            if(buffor == "") break;
            ValidateLineMicroDvd(buffor, pattern, lineNumber, byFps, start);
            std::regex_match(buffor, matches, pattern);
            start = std::atoi(matches[1].str().c_str());
            *out << "{" << std::atoi(matches[1].str().c_str()) + byFps << "}{" <<
                 std::atoi(matches[2].str().c_str()) + byFps << "}" << matches[3].str() << "\n";
            lineNumber++;
        }
